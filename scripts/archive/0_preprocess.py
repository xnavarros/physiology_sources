...
def load_and_prepare_raw(data_fname, params):
    ...
    # Set channel types for non-EEG channels based on standard names
    ...
    # --- RECOMMENDED REFERENCING SCHEME ---
    # Check if the original reference channel 'A2' needs to be added.
    # For some subjects, it might already exist as a data channel.
    ...
    # Set the EEG reference to average. This is done in-place.
    filt_raw.set_eeg_reference(ref_channels='average')
    
    # Now that referencing is done, set the montage.
...
def run_preprocessing_pipeline(raw_VS, raw_LD, events_VS, events_LD, params, ar_params):
    ...
    # REMOVED proj=True as the reference is now baked into the data
